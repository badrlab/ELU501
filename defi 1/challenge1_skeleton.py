...
draw_graph(G, node_attribute=college, list_of_values_of_attributes=list_of_different_attribute_values(college))
...
sorted_nodes_by_degree=sorted(nx.degree(G).items(), key=lambda t: t[1])
...
print("U19886 has %d neighbors" % len(G.neighbors('U19886')))
print("U19886 has %d neighbors" % G.degree('U19886'))

if 'U19886' in college:
    print("U19886's college(s): %s" % str(college['U19886']))
print("Her/his neighbors'colleges are:")
for n in G.neighbors('U19886'):
    if n in college:
        print("--- U19886's neighbor %s went in college(s): %s" % (n,str(college[n])))
    else:
...
#%%%
# --------------------- Now your turn -------------------------------------#
# Let us say you are U19886 and you want to work at Google.
...
pas=[]
for i in lis:
    path_temp=nx.shortest_path(G,'U19886',i)
    dis_temp=len(nx.shortest_path(G,'U19886',i))
    if d>=dis_temp:
        d=dis_temp
        for i in pas:
            if len(i)>d:
...
print(len(lis))
for i in pas :
    if i[-1] in college.keys(): print(college[i[-1]])
    #if i[-1] in location.keys(): print(location[i[-1]])
    #if i[-1] in employer.keys(): print(employer[i[-1]])
